...
#include"ray.hpp"
#include<fstream>
const double hit_sphere(const vec3& center, const double radius, const ray& r){
	...
	if (discriminant < 0) {
		...
		return (-b - std::sqrtf(discriminant)) / (2.0*a);
	}

}
const vec3 color(const ray& r){
	vec3 c{ {0,0,-1} };
	double t = hit_sphere(c, 0.5f, r);

	if (t > 0) {
		vec3 n = (r.point_at_param(t) - c).unit();
