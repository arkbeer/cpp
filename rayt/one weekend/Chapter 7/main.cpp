...
ark::Random rnd;
rayt::vec3 random_in_unit_sphere(){
    rayt::vec3 p;
    while(true){
        p=2.0*rayt::vec3(rnd.Rand(),rnd.Rand(),rnd.Rand())-rayt::vec3(1,1,1);
...
rayt::vec3 color(const rayt::ray& r, const std::shared_ptr<rayt::hitable>& world){
    rayt::hitinfo info;
    if(world->hit(r, 0.01, std::numeric_limits<float>::max(), info)){
        rayt::vec3 target=info.p+info.normal+random_in_unit_sphere();
        return 0.5*color(rayt::ray(info.p,target-info.p),world);
    }else{
...
int main(){
    ...
    for(int j=ny-1;j>=0;--j){
        for(int i=0;i<nx;++i){
            ...
            col=rayt::vec3(std::sqrt(col.x()),std::sqrt(col.y()),std::sqrt(col.z()));
